...
DWORD WINAPI framing_thread5(_In_ LPVOID lpParameter);
...
LRESULT TvWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
        ...
        CreateThread(NULL, 0, framing_thread5, NULL, 0, NULL);
		break;
...
DWORD WINAPI usb_reading_thread(_In_ LPVOID lpParameter)
{
    ...
    // (This effectively pins the buffers in memory so they can't be swapped out by the memory manager and so incoming USB data can be DMAed into them.)
    //
...
DWORD WINAPI framing_thread5(_In_ LPVOID lpParameter) {
    ...
    while (true) {
        ...
        auto previousSample = buf[0];
        while (true) {
            // if we're skipping pixels we may have gone through all of them in the buffer before we run out of the loop.
            int bufIdx = (int)(pxcnt - nextBufferStart);
            if (bufIdx >= bufWorkingBytes) {
                break;
            }

            auto sample = buf[bufIdx];
...
// Right. What are we trying to do here. So there are a couple of things right? One is doing a DLL that figures out where the sync pulse is. Effectively it's trying to figure out the correction factor that we need to apply to bring our clock in line with the one being generated by the video producer.
// How will it do that? Well, it could do it a few ways. I guess the easiest way would be to have a fractional pixel clock. We assume that the input pixel clock is accurate and ticks one tick so that the video lines are always a perfect integral number of samples. And then we have our clock, which isn't quite accurate and instead runs a bit too fast or slow and so instead of ticking one pixel at a time, we actually tick a little bit more or less, with the side effect being that the pixels we're feeding to the next layer are actually some fractional thing that we need to compute by interpolation.
